...
def solve(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, BATCH: int, M: int, N: int, K: int):
    BLOCK_SIZE_BATCH, BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K = 4, 64, 128, 32
...
if __name__ == "__main__":
    ...
    b = torch.ones((B, K, N), device="cuda", dtype=torch.float16)

    torch_result = torch.matmul(a, b)

    triton_result = torch.empty((B, M, N), device="cuda", dtype=torch.float16)
