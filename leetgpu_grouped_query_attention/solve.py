...
def grouped_query_attn_kernel(
    ...
    q_ptr = tl.make_block_ptr(
        base=q,
        shape=(num_q_heads, seq_len, head_dim),
        strides=(seq_len * head_dim, head_dim, 1),
        offsets=(pid_0, pid_1 * BLOCK_SIZE_SEQ, 0),
        block_shape=(1, BLOCK_SIZE_SEQ, BLOCK_SIZE_D),
        order=(2, 1, 0),
    )
    q_block = tl.load(q_ptr, boundary_check=(0, 1, 2), padding_option="zero")

    k_ptr = tl.make_block_ptr(
        base=k,
        shape=(num_kv_heads, seq_len, head_dim),
        strides=(seq_len * head_dim, head_dim, 1),
        offsets=(pid_0 // (num_q_heads // num_kv_heads), 0, 0),
        block_shape=(1, BLOCK_SIZE_SEQ, BLOCK_SIZE_D),
        order=(2, 1, 0),
    )
    ...
    top = tl.zeros((1, BLOCK_SIZE_SEQ, BLOCK_SIZE_D), dtype=tl.float32)
    bottom = tl.zeros((1, BLOCK_SIZE_SEQ, 1), dtype=tl.float32)
    max_elem = tl.full((1, BLOCK_SIZE_SEQ, 1), value=-float("inf"), dtype=tl.float32)

    for j in range(tl.cdiv(seq_len, BLOCK_SIZE_SEQ)):
        k_block = tl.load(k_ptr, boundary_check=(0, 1, 2), padding_option="zero")
        v_block = tl.load(v_ptr, boundary_check=(0, 1, 2), padding_option="zero")
        qk = tl.dot(q_block, tl.trans(k_block, (0, 2, 1)), input_precision="ieee") * REV_SQRT_D
        k_row_offsets = j * BLOCK_SIZE_SEQ + tl.arange(0, BLOCK_SIZE_SEQ)
        k_row_mask = k_row_offsets < seq_len
        qk = tl.where(k_row_mask[None, None, :], qk, -float("inf"))

        new_max_elem = tl.maximum(max_elem, tl.max(qk, axis=2, keep_dims=True))
        ...
        bottom = bottom * compensation + tl.sum(new_weight, axis=2, keep_dims=True)
        max_elem = new_max_elem

        k_ptr = tl.advance(k_ptr, (0, BLOCK_SIZE_SEQ, 0))
        v_ptr = tl.advance(v_ptr, (0, BLOCK_SIZE_SEQ, 0))
    ...
    tl.store(outut_ptr, result, boundary_check=(0, 1, 2))
