...
2. [load] 加载scale
3. [xor] 每个4bit的最高位取反, 相当于减去8(offset=8)
4. [r_shift] 得到even位置的一半unpacked权重
5. [l_shift] 对齐odd位置权重的最高位
6. [r_shift] 得到odd位置的另一半unpacked权重
...
def unpack(packed_int8_weight: tl.tensor, BLOCK_SIZE_N: tl.constexpr, BLOCK_SIZE_K: tl.constexpr):
    packed_int4_complement_code = (packed_int8_weight ^ 0x88).to(tl.int8)
    int_even_pos = packed_int4_complement_code >> 4
    int_odd_pos = (packed_int4_complement_code << 4) >> 4
    return tl.interleave(int_even_pos, int_odd_pos)
...
def dequant(
    ...
    w = unpacked_int8_weight.to(tl.float16) * tl.reshape(s, (BLOCK_SIZE_N, BLOCK_SIZE_K)).to(tl.float16)
    return w
