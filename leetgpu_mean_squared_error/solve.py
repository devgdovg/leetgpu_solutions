...
def partial_sum_kernel(
    ...
    diff = x_data - y_data
    squared_sum = tl.sum(diff * diff, axis=0, keep_dims=False)
    ...
    tl.store(partial_sum_ptr, squared_sum, boundary_check=(0,))


@triton.jit
def final_mse_kernel(
    partial_sum: torch.Tensor,
    mse: torch.Tensor,
...
def solve(predictions: torch.Tensor, targets: torch.Tensor, mse: torch.Tensor, N: int):
    ...
    partial_sum_kernel[grid](predictions, targets, partial_sum, N, partial_sum_size, BLOCK_SIZE_N)

    valid_partial_sum_size = triton.cdiv(N, BLOCK_SIZE_N)

    final_mse_kernel[(1,)](partial_sum, mse, valid_partial_sum_size, N, BLOCK_SIZE=partial_sum_size)


if __name__ == "__main__":
    x_len = 100_000_000
    x = (torch.rand(x_len) * 2000 - 1000).to(dtype=torch.float32, device="cuda")
    y = x + 5.0 * torch.randn(x_len, dtype=torch.float32, device="cuda")

    import torch.nn.functional as F

    torch_result = F.mse_loss(x, y)
